...
import { getDocument } from '../../firebase/firebaseFuntions/firestore'
...
const storeFirestore = useFireStores()
...
onMounted(async () => {
  const data = await storeFirestore.getDocument(route.params.id)
  properties.value = data
})

watch(properties, async (value) => {
  loading.value = true

  title.value.value = value.data?.title
  price.value.value = value.data?.price
  bedrooms.value.value = value.data?.bedrooms
  bathrooms.value.value = value.data?.bathrooms
  parking.value.value = value.data?.parking
  description.value.value = value.data?.description
  pool.value.value = value.data?.pool
  center.value = value.data.location
...
const submit = handleSubmit(async (values) => {
  const { image, ...propertiesValues } = values
  // primero validamos si viene una imagen nueva o no
  if (imagePreview.value) {
    ...
    await storeFirestore.updatePropertie(route.params.id, data)
  } else {
    const data = {
      ...propertiesValues,
      location: center.value
...
<template>
  <Heading title="edit your propertie" subtitle="Adjust and improve your property information"
    variants="container mx-auto" />

  <section class="pb-5">
    <span v-if="loading">cargando!!!!!!!!!!!!!!!!!!!!!!!!</span>
    <form v-else @submit="submit" class="mb-0 mt-6 space-y-5 rounded-lg p-4 shadow-lg sm:p-4 max-w-screen-lg mx-auto">
      ...
      <div class="relative grid grid-cols-6 gap-x-6 gap-y-10 border-b border-zinc-800 pb-2">
        ...
        <div class="col-span-6">
          ...
          <div class="w-full flex flex-col mt-2 gap-x-2">
            <span class="font-bold capitalize">actual image:</span>
            <img v-if="imagePreview" :src="imagePreview" class="w-80 h-52 rounded" alt="preview of the image" />
            <img v-else :src="properties.data?.image" class="w-80 h-52 rounded" alt="preview of the image" />
          </div>
        </div>
        ...
        <div class="col-span-6 h-[300px] rounded">
